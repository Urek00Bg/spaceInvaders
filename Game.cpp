...
void Game::initEnemies() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        switch (enemyType) {
        case 0:
            e = make_shared<EnemyType1>(3 + i * 4, 2);
            e->setAltSymbol('w');
            break;
        case 1:
            e = make_shared<EnemyType2>(3 + i * 4, 4);
            e->setAltSymbol('m');
            break;
        case 2:
            e = make_shared<EnemyType3>(3 + i * 4, 6);
            e->setAltSymbol('*');
            break;
        case 3:
            e = make_shared<EnemyType4>(3 + i * 4, 8);
            e->setAltSymbol('+');
            break;
        }
        enemies.push_back(e);
...
void Game::input() {
    ...
    if ((GetAsyncKeyState(' ') & 0x8000) && bulletCooldown == 0) {
        bullets.emplace_back(player.getX(), player.getY() - 1, 1, LIGHT_GREEN, L'|');
